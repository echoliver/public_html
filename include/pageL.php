...
class Core_Lib_Page
{
    ...
    public function __construct($data = array())
    {
        ...
        $this->keyword			= !empty($data['kw']) ? $data['kw'] : '龙劲舟';
...
//		  return '<a href="' . $this->_set_link($page) . '">' . $text . '</a>' . "\n";
            break;
        }
    }
     /*
      *自定义获取连接
     */
    protected function _set_link($kw,$page,$text)
    {
	$start=($page-1)*10;
	$this->url=$this->parameter."?kw=$kw&start=$start";
...
//        $parse = parse_url($url);
        if(isset($parse['query'])) {
            parse_str($parse['query'],$params);//获取url查询字符
            unset($params[$this->page_name]);
            $url   =  $parse['path'].'?'.http_build_query($params);//生成url
        }
        if(!empty($params))
        {
            $url .= '&';
...
/*    public function last_page($name = '最后一页')
    ...
    public function up_page($name = '上一页')
    {
        if($this->now_page != 1)
        {
            return "<p>".$this->_set_link($this->keyword,$this->now_page - 1, $name);
        }
        return '<p>';
    }
    ...
    public function down_page($name = '下一页')
    {
        ...
        {
            return $this->_set_link($this->keyword,$this->now_page + 1, $name)."</p>";
        }
        return '</p>';
    }
...
//        $return .= $this->first_page();
        ...
        for ($i = $begin; $i <= $begin + $plus * 2;$i++)
        {
            ...
            {
                $return .= $this->_set_link($this->keyword,$i, $i) . "\n";
            }
        }
        $return .= $this->down_page();
